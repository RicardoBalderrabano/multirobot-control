# #!/usr/bin/env python3
# """
# Input/Output Linearization Controller (Global/OptiTrack Frame)
# FIX: QoS set to BEST_EFFORT to match OptiTrack publisher.
# """
...
#         # --- DEFINING COMPATIBLE QoS PROFILE ---
#         # Matches: Reliability=BEST_EFFORT, Durability=VOLATILE
...
#         # Subscribe using the custom QoS profile
#         self.sub_pose = self.create_subscription(
#             PoseStamped,
#             self.pose_topic,
#             self.pose_callback,
#             optitrack_qos  # <--- APPLIED HERE
#         )
...
#         self.get_logger().info(f"--- OptiTrack Controller Started (Best Effort) ---")
#         self.get_logger().info(f"Listening to: {self.pose_topic}")
...
#         self.yB_goal = msg.y
#         self.has_goal = True
#         self.get_logger().info(f"New Global Goal: ({self.xB_goal:.2f}, {self.yB_goal:.2f})")

#     def control_loop(self):
#         # 1. Safety Watchdog
#         current_time = self.get_clock().now().nanoseconds / 1e9
#         if self.pose_received and (current_time - self.last_pose_time) > 0.5:
#             self.get_logger().warn("OptiTrack timeout! Stopping.")
#             self.stop_robot()
#             return
...
#             self.get_logger().info(f"Goal Reached (Error: {dist_error:.3f}m)")
#             self.stop_robot()
#             self.has_goal = False
#             return
