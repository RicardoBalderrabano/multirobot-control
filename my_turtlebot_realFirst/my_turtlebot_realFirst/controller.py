...
from geometry_msgs.msg import Pose2D  # Optitrack
...
class IOLinearizationController(Node):
    def __init__(self):
        ...
        self.declare_parameter('use_optitrack', False)  # Default to odometry OPTITRACK
        self.declare_parameter('optitrack_topic', '/burger1/optitrack_pose2d') 
        ...
        # Get the parameters
        self.use_optitrack = self.get_parameter('use_optitrack').value
        self.optitrack_topic = self.get_parameter('optitrack_topic').value
        
        # Optitrack variables
        self.optitrack_data_valid = True
        ...
        # OptiTrack subscription (only if enabled)
        if self.use_optitrack:
            ...
            self.get_logger().info(f'Using OptiTrack data from: {self.optitrack_topic}')
        else:
            self.get_logger().info('Using robot odometry data')
    ...
    def optitrack_callback(self, msg):
        ...
        # Extract position and orientation directly
        self.x = msg.x
        self.y = msg.y
        self.theta = msg.theta  # Already in radians
        
        # Calculate point B position
        self.xB = self.x + self.b * np.cos(self.theta)
        self.yB = self.y + self.b * np.sin(self.theta)
        
        # Log first 5 messages, then every 20th
        if not hasattr(self, 'optitrack_msg_count'):
            self.optitrack_msg_count = 0
        self.optitrack_msg_count += 1
        
        if self.optitrack_msg_count <= 5 or self.optitrack_msg_count % 20 == 0:
            self.get_logger().info(
                f'[OptiTrack {self.optitrack_msg_count}] Pose: ({self.x:.3f}, {self.y:.3f}) | '
                f'Theta: {np.degrees(self.theta):.1f}° | '
                f'Point B: ({self.xB:.3f}, {self.yB:.3f})'
            )

    def check_optitrack_status(self):
        
        time_since = (self.get_clock().now() - self.last_optitrack_time).nanoseconds * 1e-9
        if not self.optitrack_data_valid:
            self.get_logger().warn('Still waiting for first OptiTrack message...')
        elif time_since > 1.0:
            self.get_logger().warn(f'No OptiTrack data for {time_since:.1f} seconds!')
            self.optitrack_data_valid = False
    ...
    def control_loop(self):
        # ADD OPTITRACK CHECK
        if self.use_optitrack and not self.optitrack_data_valid:
            self.get_logger().warn('Skipping control loop: Waiting for valid OptiTrack data')
            return
        ...
        try:
            ...
            if abs(v) > 0.01 or abs(omega) > 0.01:
                self.get_logger().info(
                    f'[CONTROL] Commands: v={v:.3f}, ω={omega:.3f} | '
                    f'Position: ({self.x:.3f}, {self.y:.3f}) | '
                    f'Target: ({self.xB_desired:.3f}, {self.yB_desired:.3f})'
                )
