...
class MultiKalmanObserver(Node):
    def __init__(self):
        ...
        # --- Filter Gains & Tuning (Adaptive KF) ---
        self.q_base = 0.2        
        self.ema_alpha = 0.4       
        
        # --- Dynamic Measurement Noise Parameters ---
        ...
        self.get_logger().info("--- STARTING ROBOT DISCOVERY (ADAPTIVE LINEAR KF) ---")
        time.sleep(1.0) 
    ...
    def sync_lidar_callback(self, scan_msg, obs_pose_msg):
            ...
            for match in possible_matches:
                ...
                if not t['initialized']:
                    ...
                    current_r = min(self.r_meas_max, self.r_meas_base + (t['missed_frames'] * self.r_meas_expansion))

                    # Kalman update using adaptive current_r
                    K = t['p'] @ np.linalg.inv(t['p'] + np.eye(2) * current_r)
