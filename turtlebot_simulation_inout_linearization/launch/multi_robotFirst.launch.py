...
def _make_namespaced_bridge_yaml(original_yaml_path, namespace):
    """
    Create a temporary YAML file from original bridge YAML,
    with ROS topics namespaced and mapped to unique Gazebo topics.
    """
    ...
    for entry in bridge_config:
        ...
        # DEBUG
        print(f"Processing: {new_entry.get('ros_topic_name')}")
        ...
        if 'gz_topic_name' in new_entry:
            ...
            if gz_topic == "cmd_vel":
                ...
                new_entry['gz_topic_name'] = f"/model/{namespace}/odometry"  # FIXED
            elif gz_topic == "tf":
    ...
    for entry in namespaced_config:
        print(f"ROS: {entry.get('ros_topic_name')} -> GZ: {entry.get('gz_topic_name')}")
    print("=========================================")
...
def _make_namespaced_sdf(original_sdf_path, namespace):
    ...
    # Replace ALL occurrences of topic names
    replacements = {
        '<topic>/model/__model__/cmd_vel</topic>': f'<topic>/model/{namespace}/cmd_vel</topic>',
        '<topic>joint_states</topic>': f'<topic>/world/default/model/{namespace}/joint_state</topic>',
    ...
    for old, new in replacements.items():
        if old in sdf_content:
            sdf_content = sdf_content.replace(old, new)

    # CRITICAL FIX: Replace the existing odometry configuration
    # Find and replace the existing odom_topic configuration
    sdf_content = sdf_content.replace(
        '<odom_topic>odom</odom_topic>',
        f'<odom_topic>/model/{namespace}/odometry</odom_topic>'
    )
    
    # Also replace any other existing odometry configuration
    sdf_content = sdf_content.replace(
    ...
    # DEBUG: Print the fixed plugin section
